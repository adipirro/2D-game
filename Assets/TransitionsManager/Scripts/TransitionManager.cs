...
public class TransitionManager : MonoBehaviour
{
    ...
    private void DoSplashScreens()
    {
        ...
        fadingTexture.GetComponent<GUITexture>().texture = PitchBlackTexture;
        fadingTexture.GetComponent<GUITexture>().color = new Color(0.5f, 0.5f, 0.5f, 0.5f);
        fadingTexture.GetComponent<GUITexture>().pixelInset = new Rect(-PitchBlackTexture.width / 2, -PitchBlackTexture.height / 2, PitchBlackTexture.width, PitchBlackTexture.height);
    ...
    private void SetCurrentSplashScreenTextures()
    {
        ...
        {
            ssBackground.GetComponent<GUITexture>().texture = CustomSplashScreenBackgrounds[ssIndex];
            ssBackground.GetComponent<GUITexture>().color = new Color(0.5f, 0.5f, 0.5f, 0.5f);
            ssBackground.GetComponent<GUITexture>().pixelInset = new Rect(-CustomSplashScreenBackgrounds[ssIndex].width / 2, -CustomSplashScreenBackgrounds[ssIndex].height / 2, CustomSplashScreenBackgrounds[ssIndex].width, CustomSplashScreenBackgrounds[ssIndex].height);
        }
        else
        {
            ssBackground.GetComponent<GUITexture>().texture = WhiteTexture;
            ssBackground.GetComponent<GUITexture>().color = BackgroundColour;
            ssBackground.GetComponent<GUITexture>().pixelInset = new Rect(-WhiteTexture.width / 2, -WhiteTexture.height / 2, WhiteTexture.width, WhiteTexture.height);
        }
        ...
        ssLogo.GetComponent<GUITexture>().texture = SplashScreenContents[ssIndex];
        float myScale = 1f;
        if (ScaleContentTextures)
            myScale = scale;
        ssLogo.GetComponent<GUITexture>().pixelInset = new Rect(-((SplashScreenContents[ssIndex].width * myScale) / 2), -((SplashScreenContents[ssIndex].height * myScale) / 2), SplashScreenContents[ssIndex].width * myScale, SplashScreenContents[ssIndex].height * myScale);
        ssLogo.GetComponent<GUITexture>().color = new Color(0.5f, 0.5f, 0.5f, 0.5f);
    ...
    private void SetCurrentStoryTextures()
    {
        ...
        ssLogo.GetComponent<GUITexture>().texture = ClickThroughStory[ssIndex];
        float myScale = 1f;
        if (ScaleContentTextures)
            myScale = scale;
        ssLogo.GetComponent<GUITexture>().pixelInset = new Rect(-((ClickThroughStory[ssIndex].width * myScale) / 2), -((ClickThroughStory[ssIndex].height * myScale) / 2), ClickThroughStory[ssIndex].width * myScale, ClickThroughStory[ssIndex].height * myScale);
        ssLogo.GetComponent<GUITexture>().color = new Color(0.5f, 0.5f, 0.5f, 0.5f);
        ssLogo.transform.position = zeroPosition;
        ssLogo.transform.localScale = noScalingScale;
    }
    ...
    void Update()
    {
        ...
        {
            color = fadingTexture.GetComponent<GUITexture>().color;
            if (fadingIn)
            {
                ...
                fadingTexture.GetComponent<GUITexture>().color = color;
            }
            else if (fadingOut)
            {
                color.a += Time.deltaTime / (SplashScreenFadingTime * 2f);
    ...
    private void SetGamePaused()
    {
        ...
        if (pauseTexture == null && PauseMenu == null)
        {
            ...
            pauseTexture.GetComponent<GUITexture>().texture = PauseTexture;
            float myScale = 1f;
            if (ScaleContentTextures)
                myScale = scale;
            pauseTexture.GetComponent<GUITexture>().color = new Color(0.5f, 0.5f, 0.5f, 0.5f);
            pauseTexture.transform.position = pausePosition;
            pauseTexture.GetComponent<GUITexture>().pixelInset = new Rect(-((PauseTexture.width * myScale) / 2), -((PauseTexture.height * myScale) / 2), PauseTexture.width * myScale, PauseTexture.height * myScale);
